...
export function StudentIDCard({ data, className }: { data: StudentIDData; className?: string }) {
  ...
  const qr = `https://api.qrserver.com/v1/create-qr-code/?size=100x100&data=${encodeURIComponent(
    JSON.stringify({ id: data.id, name: data.name, contact: data.contact || "" }),
  )}`;
    ...
    >
      ...
      <div className="pr-4">
        <div className="mb-2 flex items-center gap-2 text-primary">
          ...
          <span className="text-xs font-semibold tracking-wide uppercase">{hostel}</span>
        </div>

        <div className="flex items-center gap-3">
          <div className="h-16 w-16 overflow-hidden rounded-xl ring-1 ring-border">
            {data.photoDataUrl ? (
              <img src={data.photoDataUrl} alt={data.name} className="h-full w-full object-cover" />
            ) : (
              <div className="flex h-full w-full items-center justify-center bg-muted text-muted-foreground">ID</div>
            )}
          </div>
          <div className="min-w-0">
            <div className="truncate text-lg font-bold leading-tight">{data.name}</div>
            {data.username ? (
        ...
        <div className="mt-3 grid grid-cols-2 gap-2 text-[10px] text-muted-foreground">
          <div className="rounded-md border p-2">
            ...
            <div className="font-mono text-[11px]">{data.id.slice(-8).toUpperCase()}</div>
          </div>
          <div className="rounded-md border p-2">
